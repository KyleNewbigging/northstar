...
After `/use PROJECT [model] [agent]` binds a chat or topic, ordinary non-command Telegram messages are treated as Codex worktree requests by default. If there is no route yet, a first normal message auto-routes to `northstar` unless it mentions Zebra, in which case it routes to `zebra-dashboard`; the default lane is `codex orchestrator`. Override that route label with `NORTHSTAR_TELEGRAM_DEFAULT_MODEL` and `NORTHSTAR_TELEGRAM_DEFAULT_AGENT`. The execution model defaults to Codex and can be changed with `NORTHSTAR_TELEGRAM_AUTOMATION_MODEL=codex|spark|opus`. Live Telegram messages queue the task and immediately try to start the local worktree run. The diagnostic `/api/telegram/intake` endpoint queues without dispatch unless `autoDispatch: true` is passed. Polaris still stops for no local checkout, auth/no-API guardrail failures, blocked runs, and final review.

Loop monitoring stays active without hard-pausing progress. After the retry thresholds (`NORTHSTAR_TASK_MAX_DISPATCH_ATTEMPTS`, default `3`, or `NORTHSTAR_TASK_MAX_MODEL_ATTEMPTS`, default `2`) are reached, the task remains queued but enters a monitored cooldown for `NORTHSTAR_LOOP_MONITOR_COOLDOWN_MS` (default `600000`, or 10 minutes). Dispatchability reports `scheduler-waiting`, includes estimated retry-usage percentages when exact provider usage is unavailable, and automatically permits another attempt after the cooldown.

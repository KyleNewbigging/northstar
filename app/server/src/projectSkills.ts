...
export function ensureProjectSkillsFile(project: LocalProject) {
  const path = projectSkillsPath(project)
  if (existsSync(path)) {
    mirrorProjectSkills(project, path)
...
function mirrorProjectSkills(project: Pick<LocalProject, 'id'>, sourcePath: string) {
  try {
    const status = getDeviceFilesStatus()
    if (!status.rootExists) return

    const projectId = safeFileStem(project.id)
    const mirrorDir = join(status.rootPath, 'projects', projectId, 'skills')
    mkdirSync(mirrorDir, { recursive: true, mode: 0o700 })
    writeFileSync(join(mirrorDir, 'northstar-skills.md'), generatedMirrorContent(sourcePath), { mode: 0o600 })
  } catch {
    // Dropbox mirroring is best-effort; the canonical skills file stays under ~/.northstar.
  }
}

function generatedMirrorContent(sourcePath: string) {
  return [
    '# Northstar Skills Mirror',
    '',
    'Generated by Northstar from the canonical runtime skills file.',
    'Do not edit this Dropbox copy in v1; edits are not reverse-synced.',
    '',
    `Canonical file: ${displayPath(sourcePath)}`,
